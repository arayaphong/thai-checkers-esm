...
export const boardFromInitialSetup = (initialSetup) => {
    if (initialSetup === undefined || initialSetup === null || typeof initialSetup.board !== 'string') {
...
export const moveRecordFromMove = (index, move) => ({
    ...
    path: (move.path && move.path.length > 0 ? move.path : [move.from, move.to]).map((pos) => pos.toString()),
});
...
export const moveRecordMatches = (move, record) => {
    const movePath = (move.path && move.path.length > 0 ? move.path : [move.from, move.to])
    ...
    const capturedMatches = move.captured.length === record.captured.length
        && move.captured
            .map((pos) => pos.toString())
            .toSorted()
            .every((sq, i) => sq === [...record.captured].toSorted()[i]);
    return move.from.toString() === record.from
...
const detectInputShape = (json) => {
    ...
    throw new Error('Unrecognized input shape: expected a demo/setup object with "pieces" or a saved session with "format"');
};
...
export class GameDriver {
    ...
    getState() {
        ...
        const winner = noLegalMoves ? (player === PieceColor.WHITE ? PieceColor.BLACK : PieceColor.WHITE) : null;
        const isDraw = oneDameEach;
        const drawReason = oneDameEach ? 'ONE_DAME_EACH' : null;
        const drawWarning = (!isGameOver && isImmediateDraw(board, player))
            ? { reason: 'DRAW_POSSIBLE' }
            : null;
        return {
            board,
            player,
    ...
    playMovePosition(from, to, choice) {
        ...
        if (!Number.isInteger(choice) || choice < 1 || choice > candidates.length) {
            throw new RangeError(
                `Choice ${choice} out of range; valid range is 1-${candidates.length}`,
            );
        }
    ...
    toJSON() {
        const moveSequence = this.#history
            .slice(1)
            .map((entry) => entry.moveRecord);
        return {
    ...
    load(json) {
        ...
        if (json.format !== SESSION_FORMAT) {
            throw new Error(`Cannot load input with format ${String(json.format)}; expected ${SESSION_FORMAT}`);
        }
