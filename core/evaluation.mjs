...
// view (y=7 is White's back rank per core/board.js HOME_ROWS, y=0 is White's
// promotion row) and BLACK's table is the same values with the row mirrored
// (y -> 7-y), per PLAN.md's "ใช้ symmetry: ตารางดำ mirror จากตารางขาว".
//
// Pion: small back-rank guard bump at y=7, a plain "just stepped up" square
// at y=6, then a smooth (not spiky) climb toward the promotion side, plus a
// center-file preference. Realized range here is -12 to +19 (PLAN.md's
// target is "≈ -15 to +20" — these are starting values for Phase 8 tuning,
// not final).
const PION_ROW_BONUS = [12, 10, 8, 6, 4, 2, -4, 5]; // index: y = 0 (promotion side) .. 7 (back rank)
 ...
 * Phase 4 (PST) made most positions no longer tie on material alone (see
 * PLAN.md's Phase 4 work log for the node-count blowup this caused, and the
 * Phase "move-ordering fix" entry for this function's own before/after).
...
// PLAN.md's v1 suggests full move generation (Game.from(board,
// opponent).getMoves()) for this check; benchmarking showed it roughly
// doubles time per search node (see PLAN.md's Phase 6 work log), so this
// goes straight to the "optimize later with direct attack masks" fallback it
// names, and to Phase 9's "replace... with direct bitboard attack check".
//
// The direct check only needs to look for DAME attackers, never pions: a
// pion capturing "candidate" would have to sit at the candidate's y-1 (its
// own forward direction lands one row closer to its target), but "passed"
// requires every enemy pion to have a *larger* y-distance-from-target than
// that (white: candidate.y < enemy.y; symmetric for black) — so a pion
// attacker would have to simultaneously violate the passed condition it just
// satisfied. Only a flying dame, unconstrained by that forward-only
// adjacency, can threaten a genuinely passed pion.
...
// would need in order to capture this pion — PLAN.md's "blocked-capture
// support". Deliberately does *not* score a friendly piece merely standing
// diagonally behind the pion with no attacker to block (PLAN.md's explicit
// non-goal: no broad "supported pion"), nor an empty landing square with an
// attacker present (that's a live tactical threat for Quiescence/Negamax to
// find, not Structure's job).
