...
export const createPieceElement = ({ color, rank }, { selected = false, mandatoryCapture = false, moveableHint = false } = {}) => {
  const pieceEl = h('div', `${boardClassMap.pieceBase} ${
  ...
  const inner = h('div', color === 'white' ? boardClassMap.pieceInnerWhite : boardClassMap.pieceInnerBlack);
  if (rank === 'king') {
